...
class MyContract2 extends SmartContract {
  ...
  @method
  public async setValue(value: Field, address: PublicKey, actionState: Field) {
...
describe("actionState error", () => {
  ...
  it(`should update`, async () => {
    ...
    const account = Mina.getAccount(zkKey1);
    const actionState = account.zkapp?.actionState[0];
    if (!actionState) {
      throw new Error("actionState not found");
    }
    ...
    const tx = await Mina.transaction(
      ...
      async () => {
        await contract2.setValue(Field(1), zkKey1, actionState);
      }
    );
    await tx.prove();
