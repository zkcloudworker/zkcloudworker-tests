...
import { accountBalanceMina, blockchain, initBlockchain } from "zkcloudworker";
import { GASTANKS } from "../env.json";

const chain = "local" as blockchain;

export interface ContractDeployProps extends Exclude<DeployArgs, undefined> {
  value: Field;
}
...
export class MyContract1 extends SmartContract {
  ...
  async deploy(props: ContractDeployProps) {
    await super.deploy(props);
    this.value.set(props.value);
    this.account.zkappUri.set("vk-proof");
  ...
  it("should deploy and prove", async () => {
    ...
    let tx = await Mina.transaction(
      ...
      async () => {
        ...
        await zkApp1.deploy({ value: Field(0) });
      }
    );
    await tx.prove();
    let sendTx = await (
