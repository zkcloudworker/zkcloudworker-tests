type QueryType = "balance" | "account" | "transaction";
...
export async function getTransactionFromGraphQL(params: {
  txnId: string;
  mina: string[];
}) {
  const { txnId, mina } = params;
  if (mina.length === 0) throw new Error("no mina endpoints provided");
  const transaction = await fetchTransactionInternal({
    txnId,
    mina,
    queryType: "transaction",
  });
  console.log("getTransactionFromGraphQL transaction:", transaction);
  //const balance = (account as any)?.account?.balance?.total;
  return transaction;
}
...
export async function getAccountFromGraphQL(params: {
  ...
  //console.log("account:", (account as any)?.account?.account);
...
async function fetchTransactionInternal(params: {
  txnId: string;
  mina: string[];
  timeout?: number;
  queryType: QueryType;
}) {
  const { txnId, mina, timeout, queryType } = params;
  const query =
    queryType === "transaction"
      ? transactionQuery(txnId)
      : transactionQuery(txnId);
  let [response, error] = await makeGraphqlRequest({
    query,
    mina,
  ...
  console.log("response:", response);
  if (error !== undefined) return { transaction: undefined, error };

  const transaction = (response as any)?.data;
  if (!transaction) {
    return {
      transaction: undefined,
      error: {
        statusCode: 404,
        statusText: `fetchTransaction: Transaction with hash ${txnId} does not exist.`,
      },
    };
  }
  // transaction successfully fetched - add to cache before returning
  return {
    transaction,
    error: undefined,
  };
...
async function makeGraphqlRequest(params: {
  ...
  const makeRequest = async (url: string) => {
    ...
    console.log("makeGraphqlRequest body:", body);
    try {
...
async function checkResponseStatus(
  ...
  if (response.ok) {
    ...
    console.log("checkResponseStatus jsonResponse:", {
...
/*
const transactionQuery = (txnHash: string) => `{
  query(hash: "${txnHash}") 
  {
    blockHeight
    failureReason {
...
*/
const transactionQuery = (hash: string) => `{
