...
class IndexedMerkleMapBase {
  ...
  constructor() {
    ...
    let firstNode = BlakeHash([
      Bytes32.fromBigInt(firstLeaf.key),
      Bytes32.fromBigInt(firstLeaf.value),
      Bytes32.fromBigInt(firstLeaf.nextKey),
    ]).toBigInt();
    let root = Nodes.setLeaf(nodes, 0, firstNode);
    this.root = Bytes32.fromBigInt(root);
    this.length = Field(1);

    this.data = Unconstrained.from({
      nodes,
      sortedLeaves: [firstLeaf],
    });
  }
  ...
  _computeRoot(
    ...
    class Witness extends Struct({
      witness: Provable.Array(Bytes32, this.height - 1),
    }) {}

    let witness__ = witness
      ? new Witness({ witness })
      : Provable.witness(Witness, () => {
          ...
          return new Witness({ witness: witness.map(Bytes32.fromBigInt) });
        });

    const witness_ = witness__.witness as Bytes32[];
...
class Leaf extends Struct({
  ...
  static toStored(leaf: Leaf): StoredLeaf {
    return {
      key: (leaf.key as Bytes32).toBigInt(),
      value: (leaf.value as Bytes32).toBigInt(),
      nextKey: (leaf.nextKey as Bytes32).toBigInt(),
...
function assertStrictlyBetween(
  ...
  let x_ = Field(0);
  for (let i = 0; i < 32; i++) {
    x_ = x_.add(x.bytes[i].value);
  }
  x_.assertNotEquals(0n, message ?? "0 is not in any strict range");
